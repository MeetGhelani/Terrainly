...
export const HelpModal = ({ isOpen, onClose }: HelpModalProps) => {
  ...
  useEffect(() => {
    ...
    // Handle tab centering manually to prevent flickering/jumping
    if (tabsRef.current) {
  ...
  const TabButton = ({ id, label, icon: Icon }: { id: Tab; label: string; icon: any }) => (
    <button
      ...
      data-tab-id={id}
      className={cn(
        "flex items-center gap-2 md:gap-3 px-4 md:px-6 py-2 md:py-3 rounded-xl md:rounded-2xl group whitespace-nowrap",
        activeTab === id 
          ? "bg-accent text-white shadow-lg shadow-accent/20" 
          : "text-text-secondary hover:bg-bg-elevated hover:text-text-primary"
      )}
    >
      <Icon className={cn("w-3.5 h-3.5 md:w-4 md:h-4", activeTab === id ? "text-white" : "group-hover:text-accent")} />
      <span className="text-[10px] md:text-[11px] font-bold uppercase tracking-widest">{label}</span>
    </button>
  );

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 pb-24 md:pb-6 animate-in fade-in duration-500">
      {/* Backdrop */}
      <div 
      ...
      {/* Modal Content */}
      <div className="relative w-full max-w-5xl h-[90vh] md:h-[85vh] bg-bg-panel border border-border-subtle rounded-[2rem] md:rounded-[3rem] shadow-[0_30px_100px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
        
        {/* Header Area */}
        <div className="bg-bg-elevated/20 p-5 md:p-8 border-b border-border-subtle flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 md:gap-5">
            <div className="w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-[1.25rem] bg-accent/10 border border-accent/20 flex items-center justify-center relative shrink-0">
              <Sparkles className="w-7 h-7 text-accent" />
              <div className="absolute -top-1 -right-1 w-3 h-3 bg-accent rounded-full animate-ping" />
            </div>
            <div className="min-w-0">
              <h2 className="text-lg md:text-2xl font-space font-bold text-text-primary tracking-tight truncate">Studio Masterclass</h2>
              <div className="flex items-center gap-2 mt-0.5 md:mt-1">
                <span className="px-1.5 py-0.5 rounded-full bg-accent/20 text-accent text-[8px] font-bold uppercase tracking-tighter">Tutorial Mode</span>
                <div className="h-1 w-1 rounded-full bg-text-muted hidden sm:block" />
                <p className="text-[9px] text-text-secondary uppercase tracking-[0.1em] md:tracking-[0.2em] font-medium truncate hidden sm:block">Become a terrain artist in minutes</p>
              </div>
            </div>
          </div>
          <button 
            onClick={onClose}
            className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-bg-surface border border-border-subtle flex items-center justify-center text-text-secondary hover:text-text-primary hover:border-text-muted transition-all active:scale-90"
          >
        ...
        <div 
          ref={tabsRef}
          className="px-5 md:px-8 py-3 md:py-4 bg-bg-surface/30 border-b border-border-subtle flex items-center gap-2 overflow-x-auto no-scrollbar"
        >
        ...
        <div 
          ...
          className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar bg-gradient-to-b from-transparent to-bg-panel/50"
        >
          ...
          {activeTab === "welcome" && (
            <div className="max-w-3xl mx-auto space-y-12 animate-in slide-in-from-bottom-4 duration-500">
              <div className="text-center space-y-4">
                ...
                <h3 className="text-2xl md:text-4xl font-space font-bold text-text-primary">Create Your Masterpiece</h3>
                <p className="text-sm md:text-base text-text-secondary leading-relaxed max-w-xl mx-auto px-4">
        ...
        <div className="p-8 bg-bg-elevated/30 border-t border-border-subtle flex items-center justify-between shrink-0">
          ...
          <button 
            onClick={onClose}
            className="flex items-center gap-2 md:gap-3 px-6 md:px-8 py-2.5 md:py-3 rounded-full bg-text-primary text-bg-panel hover:bg-accent hover:text-white transition-all font-bold text-[9px] md:text-[10px] uppercase tracking-widest whitespace-nowrap"
          >
            Enter the Studio <ChevronRight className="w-3.5 h-3.5 md:w-4 md:h-4" />
          </button>
        </div>
      </div>
    </div>
